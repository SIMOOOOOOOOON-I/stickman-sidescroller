...
# drawing text is slow (especially in a web browser), so every text is
# drawn only once and then remembered
text_cache = {}


def get_text(text_font, text, color):
    key = (id(text_font), text, color)
    if key not in text_cache:
        if len(text_cache) > 300:
            text_cache.clear()
        text_cache[key] = text_font.render(text, True, color)
...
def remove_far_objects():
    # things far behind the player are deleted, so the game does not get slower and slower
    limit = player_rect.x - BEHIND_LIMIT
    ...
    for pickup in pickups[:]:
        if pickup.right < limit:
            pickups.remove(pickup)

    for item in weapon_items[:]:
        if item["rect"].right < limit:
            weapon_items.remove(item)

    for enemy in enemies[:]:
        if enemy["rect"].right < limit:
            enemies.remove(enemy)
...
# ---------- drawing functions ----------
def on_screen(rect, camera_x):
    # True if any part of the rect is inside the screen (or just next to it)
    return rect.right - camera_x > -60 and rect.x - camera_x < SCREEN_WIDTH + 60
...
def draw_weapon_items(camera_x):
    for item in weapon_items:
        rect = item["rect"]
        if not on_screen(rect, camera_x):
            continue
...
def draw_bosses(camera_x):
    for boss in bosses:
        ...
        level_text = get_text(tiny_font, "BOSS " + str(boss["level"]), BLACK)
...
def draw_hud():
    score_text = get_text(font, "Score: " + str(state["score"]), BLACK)
    ...
    weapon_text = get_text(tiny_font, "Weapons (L = switch, X = fire): " + names, BLACK)
    ...
    ability_text = get_text(tiny_font, "Abilities (Q = switch, Z = use): " + names, BLACK)
    ...
    if key is not None:
        ...
        status_text = get_text(small_font, ABILITIES[key]["label"] + ": " + status, BLACK)
        screen.blit(status_text, (30, 116))

    boss_text = get_text(small_font, "Next boss at: " + str(state["next_boss_level"]), BLACK)
    ...
    if effects != "":
        screen.blit(get_text(small_font, effects, BLACK), (10, 162))
    ...
    if "recruit" in state["abilities"] or len(allies) > 0:
        ...
        screen.blit(get_text(tiny_font, info, BLACK), (10, 184))


def draw_message():
    if state["message_timer"] <= 0:
        return
    text = get_text(font, state["message"], BLACK)
...
def draw_game_over():
    ...
    over_text = get_text(big_font, "GAME OVER", WHITE)
    restart_text = get_text(font, "Press R to restart", WHITE)
...
async def main():
    ...
    while running:
        ...
        # the game updates once for every 16 ms that passed, so it runs at the
        # same speed on every screen (some screens redraw 120 times a second)
        now = pygame.time.get_ticks()
        owed = owed + now - last_time
        last_time = now
        owed = min(owed, FRAME_MS * 3)
        ...
        # clock.tick makes the browser wait, so only use it on a real computer
        if not IS_WEB:
            clock.tick(FPS)
